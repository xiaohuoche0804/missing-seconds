#!/usr/bin/env python3
"""Publish chapter 12 to fanqienovel"""
...
with sync_playwright() as p:
    ...
    # Find and fill chapter number
    print("Looking for chapter number input...")
    try:
        # Try to find chapter number field - usually a number input
    ...
    try:
        title_input = page.locator('input[placeholder*="章节"]').first
        if not title_input.is_visible():
            title_input = page.locator('input').filter(has_text="").nth(1)
    ...
    page.screenshot(path="/Users/ganghonghai/Documents/openclaw-novel-workspace/publish_result.png")
    print("Screenshot saved to publish_result.png")
